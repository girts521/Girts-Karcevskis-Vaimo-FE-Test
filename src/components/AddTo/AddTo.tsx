...
const AddTo: React.FC<{ shipping: Shipping }> = ({ shipping }) => {
  ...
  return (
    <div className={styles.addToContainer}>
      {cartDataInRedux.cart.length
        ? cartDataInRedux.cart.map((item: any) => {
            return (
              <div className={styles.cartItem} key={item.option}>
                {item.option}{" "}
                <span>{`${(item.value * item.price).toFixed(2)}`}</span>
              </div>
            );
          })
        : ""}
      {totalPrice ? (
        <div className={styles.totalPrice}>
          Total price for chosen options <span> {totalPrice.toFixed(2)}</span>{" "}
        </div>
      ) : (
        ""
      )}
      <div className={styles.shipping}>
        ...
        <div className={styles.shippingPrice}>
          ...
          {shipping.method.cost.value.toFixed(2)}
        </div>
      </div>
